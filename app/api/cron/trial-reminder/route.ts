...
export async function GET(request: Request) {
  ...
  if (!canSendEmail()) {
    return NextResponse.json({ ok: true, message: ta('trialReminderCronResendSkipped') }, { status: 200 });
  }
  ...
  if (!profiles?.length) {
    return apiJsonMessage(request, 'errors.trialEligibleNone', 200, { sent: 0 });
  }
  ...
  const planPrice = formatPlanAmountForLocale('fr', 'zenith');
  ...
  for (const p of profiles) {
    ...
    const planPriceBySlug: Record<string, string> = {
      vision: formatPlanAmountForLocale('fr', 'vision'),
      pulse: formatPlanAmountForLocale('fr', 'pulse'),
      zenith: formatPlanAmountForLocale('fr', 'zenith'),
    };
