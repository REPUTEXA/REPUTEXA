...
async function run(): Promise<RunResult> {
  ...
  // ── 1. RGPD Anonymisation — entrées hors fenêtre de conservation ─────────
  //
  ...
  //   - source_info → NULL (libellé source potentiellement personnel)
  ...
  const { data: profilesRaw } = await admin
    .from('profiles')
    .select('id, establishment_name, full_name, google_review_url, business_category')
    .in('id', userIds);
  ...
  for (const entry of queue as QueueEntry[]) {
    ...
    // ── 5c. Envoi WhatsApp — étape A : amélioration service, 1/2 (pas de lien Google) ──
    let message = buildReviewQueueInitialWhatsApp({
      firstName,
      commerceName,
      lastPurchase,
    });
    const ch = challengeMap.get(userId);
      ...
      await admin
        ...
        .update({
          ...
          metadata: {
            ...
            /** Preuve du contenu envoyé (effacé à l’anonymisation 90 j. — Art. 5 RGPD / litiges). */
