 ...
 *
 * Les réponses API ne doivent pas exposer de texte métier depuis Zod : utiliser
 * {@link StripeValidationErrorCode} (+ éventuellement `details` technique).
 */

import { z } from 'zod';

/** Codes machine pour corps JSON d’erreur (pas de libellés utilisateur). */
export const StripeValidationErrorCode = {
  checkoutQueryInvalid: 'STRIPE_CHECKOUT_QUERY_INVALID',
  bulkExpansionBodyInvalid: 'STRIPE_BULK_EXPANSION_BODY_INVALID',
  previewExpansionQueryInvalid: 'STRIPE_PREVIEW_EXPANSION_QUERY_INVALID',
  syncProfileBodyInvalid: 'STRIPE_SYNC_PROFILE_BODY_INVALID',
} as const;

export type StripeValidationErrorCodeValue =
  (typeof StripeValidationErrorCode)[keyof typeof StripeValidationErrorCode];
