...
export async function POST(request: Request) {
  try {
    ...
    if (!userId || !googleConversationId || !messageText) {
      return apiJsonError(request, 'errors.googleMessages_requiredFields', 400);
    }
    ...
    if (!threadId) {
      ...
      if (insertError || !inserted?.[0]?.id) {
        ...
        return apiJsonError(request, 'errors.threadCreateFailed', 500);
      }
    ...
    if (patronWhatsApp) {
      ...
      const fromName = customerName.trim() ? customerName.trim() : t('googleMessages_whatsappAnonymous');
      const bodyText = t('googleMessages_whatsappBody', { fromName, preview });
