...
import { buildComptaBananoMonthlySummary, comptaBananoMonthlyToCsv } from '@/lib/banano/compta-banano-monthly';
...
export async function GET(request: NextRequest) {
  ...
  if (!built.ok) {
    console.error('[compta/voucher-summary/csv]', built.error);
    return apiJsonError(request, 'serverError', 500);
  }

  const csv = comptaBananoMonthlyToCsv(built.data);
  const filename = `reputexa-expert-comptable-${y}-${String(m).padStart(2, '0')}.csv`;
