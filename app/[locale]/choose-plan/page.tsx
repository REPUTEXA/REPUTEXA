...
/** Slugs de plan (localStorage / checkout), hors JSX pour le linter i18n. */
const PLAN_VISION = 'vision' as const;
const PLAN_PULSE = 'pulse' as const;
const PLAN_ZENITH = 'zenith' as const;
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-slate-50 via-white to-slate-100/40">
      <header className="flex items-center justify-between px-4 sm:px-6 h-14 border-b border-slate-200/80 bg-white/70 backdrop-blur-sm">
        ...
        <Link href="/dashboard" className="text-sm text-slate-600 hover:text-slate-900 font-medium">
          {t('pricing.choosePlan.linkDashboard')}
        </Link>
      </header>
        ...
        <div className="w-full max-w-4xl">
          <div className="text-center mb-10">
            ...
            <p className="text-slate-500 mt-2">
              {noTrial ? t('pricing.choosePlan.subtitleNoTrial') : t('pricing.choosePlan.subtitleTrial')}
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <Link
              href={
                noTrial
                  ? `/signup?mode=checkout&plan=vision&annual=0&quantity=1`
                  : `/signup?mode=trial&plan=vision`
              }
              onClick={() => noTrial && saveCheckoutIntent(PLAN_VISION, false, 1)}
              className="flex flex-col rounded-2xl border border-slate-200 bg-white p-6 hover:shadow-xl hover:border-[#2563eb]/40 transition-all"
            >
              ...
              <p className="mt-4 text-2xl font-bold text-slate-900">
                {formatPlanAmountForLocale(locale, PLAN_VISION, { monthly: true })}
              </p>
              ...
              <span className="mt-6 block w-full py-2.5 rounded-xl bg-primary text-white text-center font-semibold text-sm hover:brightness-110 active:scale-[0.98] transition-colors">
            ...
            <Link
              href={
                noTrial
                  ? `/signup?mode=checkout&plan=pulse&annual=0&quantity=1`
                  : `/signup?mode=trial&plan=pulse`
              }
              onClick={() => noTrial && saveCheckoutIntent(PLAN_PULSE, false, 1)}
              className="flex flex-col rounded-2xl border-2 border-[#2563eb] bg-white p-6 shadow-lg hover:shadow-xl relative scale-105"
            >
              ...
              <p className="mt-4 text-2xl font-bold text-slate-900">
                {formatPlanAmountForLocale(locale, PLAN_PULSE, { monthly: true })}
              </p>
              <ul className="mt-4 space-y-2 flex-1">
                <li className="flex items-center gap-2 text-sm text-slate-600">
                  <CheckCircle2 className="w-4 h-4 text-emerald-500 shrink-0" />
            ...
            <Link
              href={
                noTrial
                  ? `/signup?mode=checkout&plan=zenith&annual=0&quantity=1`
                  : `/signup?mode=trial&plan=zenith`
              }
              onClick={() => noTrial && saveCheckoutIntent(PLAN_ZENITH, false, 1)}
              className="flex flex-col rounded-2xl border-2 border-[#2563eb]/60 bg-white p-6 shadow-md hover:shadow-xl hover:border-[#2563eb] transition-all relative"
            >
              <span className="absolute -top-3 left-1/2 -translate-x-1/2 text-xs font-bold px-3 py-1 bg-primary text-white rounded-full">
              ...
              <p className="mt-4 text-2xl font-bold text-slate-900">
                {formatPlanAmountForLocale(locale, PLAN_ZENITH, { monthly: true })}
              </p>
              <ul className="mt-4 space-y-2 flex-1">
                <li className="flex items-center gap-2 text-sm text-slate-600">
                  <CheckCircle2 className="w-4 h-4 text-emerald-500 shrink-0" />
