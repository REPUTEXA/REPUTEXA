...
async function resolveActiveConversation(phone: string): Promise<{
  ...
  const { data: merchant } = await admin
    .from('profiles')
    .select('id, establishment_name, full_name, google_review_url, establishment_type, address')
...
async function handleReply1(
  ...
  const firstName    = entry.first_name ? `, ${entry.first_name}` : '';
  // Personalisation avec le dernier achat/service si disponible
  const lastPurchase = typeof entry.metadata?.last_purchase === 'string'
    ? entry.metadata.last_purchase.trim()
    : null;

  const question = lastPurchase
    ? `Comment s'est passé votre *${lastPurchase}* ? Partagez votre ressenti librement — en texte ou en vocal, comme vous préférez 🎙️`
    : `En quelques mots (ou en vocal), comment s'est passée votre expérience chez *${name}* ?`;

  const message = `Merci${firstName} ! 🙏\n\n${question}`;
...
async function handleFreeText(
  ...
  const processingMsg = `Merci pour votre retour ! ✨ Je prépare une belle version de votre avis, un instant...`;
...
async function handleModifyReview(
  ...
  const message = `Bien sûr ! 😊 Racontez-moi votre expérience à nouveau — je vous prépare une nouvelle version.`;
  await calculateHumanDelay(message, phone, incomingMsgId);
  await sendWhatsAppMessage(phone, message);
}
...
async function handlePublishReview(
  ...
  const message =
    `Parfait ! 🎉\n\nCliquez sur ce lien — votre avis sera *copié automatiquement* dans votre presse-papier, puis vous serez redirigé vers Google :\n\n👉 ${publishUrl}\n\nMerci infiniment au nom de toute l'équipe de ${buildEstablishmentName(merchant)} ! 🙏`;
...
export async function POST(request: Request) {
  try {
    ...
    if (intent === 'reply_1') {
      await handleReply1(phone, entry, merchant, incomingMsgId);
