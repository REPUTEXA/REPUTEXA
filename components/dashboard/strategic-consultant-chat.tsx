...
export function StrategicConsultantChat({ planSlug, fullPage = false }: Props) {
  const t = useTranslations('Dashboard.strategicConsultant');
  const quickQuestions = [t('quick1'), t('quick2'), t('quick3')];
    ...
    try {
      const res = await fetch('/api/consultant-chat', {
        ...
        body: JSON.stringify({
          ...
          activeEstablishmentId: activeLocation?.activeLocationId ?? 'profile',
        }),
      });
      ...
      if (!res.ok) {
        setMessages((prev) => [
          ...prev,
          { role: 'assistant', content: data.error || t('connectionError') },
        ]);
        return;
      }
      ...
      setMessages((prev) => [
        ...prev,
        { role: 'assistant', content: t('connectionFailed') },
      ]);
    } finally {
      setLoading(false);
    }
