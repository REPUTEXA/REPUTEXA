...
export async function fetchRagContext(
  ...
  // ── 1. PRIORITE ABSOLUE : legal_versioning (ne depend pas de l embedding) ──
  ...
  // ── Requetes paralleles (knowledge + code + legal + feedback) ──────────────
  const [learningRes, codeRes, legalVectorRes, feedbackRes] = await Promise.all([
