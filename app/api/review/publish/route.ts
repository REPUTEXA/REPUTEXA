...
type ApiTranslator = ReturnType<typeof createServerTranslator>;
...
export async function GET(request: Request) {
  ...
  if (!id) {
    return buildErrorPage(t('reviewPublish_errInvalidLink'), t('reviewPublish_errInvalidLinkDetail'), t, locale);
  }

  const admin = createAdminClient();
  if (!admin) {
    return buildErrorPage(
      t('reviewPublish_errServiceUnavailable'),
      t('reviewPublish_errServiceUnavailableDetail'),
      t,
      locale
    );
  }
  ...
  if (queueError || !queueRow) {
    return buildErrorPage(
      t('reviewPublish_errQueueNotFound'),
      t('reviewPublish_errQueueNotFoundDetail'),
      t,
      locale
    );
  }
  ...
  if (!reviewText) {
    return buildErrorPage(
      t('reviewPublish_errMissingText'),
      t('reviewPublish_errMissingTextDetail'),
      t,
      locale
    );
  }
  ...
  const establishmentName = merchant?.establishment_name?.trim() || t('reviewPublish_defaultMerchant');

  return buildPublishPage(reviewText, googleUrl, establishmentName, t, locale);
}
...
function buildPublishPage(
  ...
  const redirectTarget =
    googleUrl ?? 'https://www.google.com/maps/search/' + encodeURIComponent(establishmentName);

  const i18nJson = JSON.stringify({
    jsSuccessTitle: t('reviewPublish_jsSuccessTitle'),
    jsSuccessDetail: t('reviewPublish_jsSuccessDetail'),
    jsSuccessSubtitle: t('reviewPublish_jsSuccessSubtitle'),
    jsFallbackTitle: t('reviewPublish_jsFallbackTitle'),
    jsFallbackDetail: t('reviewPublish_jsFallbackDetail'),
    jsFallbackSubtitle: t('reviewPublish_jsFallbackSubtitle'),
    jsManualCopyWorking: t('reviewPublish_jsManualCopyWorking'),
    jsManualCopyDone: t('reviewPublish_jsManualCopyDone'),
  });
...
<head>
  ...
  <title>${escapeHtml(t('reviewPublish_htmlTitle'))}</title>
  <style>
...
<body>
  <div class="card">
    <div class="logo">🌟</div>
    <h1>${escapeHtml(t('reviewPublish_heading'))}</h1>
    <p class="subtitle" id="subtitle">${escapeHtml(t('reviewPublish_subtitleInitial'))}</p>
    ...
    <div class="status loading" id="status-box">
      ...
      <div class="status-text">
        <strong id="status-title">${escapeHtml(t('reviewPublish_statusLoadingTitle'))}</strong>
        <span id="status-detail">${escapeHtml(t('reviewPublish_statusLoadingDetail'))}</span>
      </div>
    </div>
    ...
    <button class="fallback-btn" id="fallback-btn" onclick="manualCopyAndRedirect()">
      ${escapeHtml(t('reviewPublish_fallbackButton'))}
    </button>

    <a class="manual-link" id="manual-link" href="${escapeHtml(redirectTarget)}" target="_blank" rel="noopener noreferrer">
      ${escapeHtml(t('reviewPublish_manualLink'))}
    </a>

    <div class="progress-wrap" id="progress-wrap" style="display:none">
      <p class="progress-label">${escapeHtml(t('reviewPublish_progressLabel'))}</p>
      <div class="progress-bar"><div class="progress-fill" id="progress-fill"></div></div>
    </div>

    <p class="powered">${escapeHtml(t('reviewPublish_poweredBy'))} <strong>${escapeHtml(t('reviewPublish_brandName'))}</strong></p>
  </div>

  <script>
    const I18N = ${i18nJson};
    const REVIEW_TEXT  = \`${escapedText}\`;
    const REDIRECT_URL = ${JSON.stringify(redirectTarget)};
    const DELAY_MS     = 1200;
    ...
    function showSuccess() {
      ...
      document.getElementById('status-title').textContent  = I18N.jsSuccessTitle;
      document.getElementById('status-detail').textContent = I18N.jsSuccessDetail;
      document.getElementById('subtitle').textContent      = I18N.jsSuccessSubtitle;
      startRedirect();
    }
    ...
    function showFallback() {
      ...
      document.getElementById('status-title').textContent  = I18N.jsFallbackTitle;
      document.getElementById('status-detail').textContent = I18N.jsFallbackDetail;
      document.getElementById('subtitle').textContent      = I18N.jsFallbackSubtitle;
    ...
    async function manualCopyAndRedirect() {
      const btn = document.getElementById('fallback-btn');
      btn.disabled = true;
      btn.textContent = I18N.jsManualCopyWorking;
      try {
      ...
      btn.textContent = I18N.jsManualCopyDone;
...
function buildErrorPage(title: string, message: string, t: ApiTranslator, locale: string): Response {
  const htmlLang = locale;
  const html = `<!DOCTYPE html>
<html lang="${escapeHtml(htmlLang)}">
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>${escapeHtml(t('reviewPublish_errorHtmlTitle'))}</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body {
