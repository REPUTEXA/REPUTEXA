...
export function AIResponseModal({ reviewId, reviewText, onClose, onResponded }: Props) {
  const ta = useTranslations('Dashboard.aiResponseModal');
    ...
    async function fetchOptions() {
      try {
        ...
        if (!res.ok) {
          ...
          throw new Error(data.error || ta('errorGenerate'));
        }
        const data = await res.json();
        if (!cancelled) setOptions(data.options ?? []);
      } catch (e) {
        if (!cancelled) setError(e instanceof Error ? e.message : ta('errorGeneric'));
      } finally {
  ...
  const handleSelect = async (option: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        throw new Error(data.error || ta('errorSend'));
      }
      onResponded();
      onClose();
    } catch (e) {
      setError(e instanceof Error ? e.message : ta('errorGeneric'));
    } finally {
      setSending(null);
    }
  };
        ...
        <div className="p-4 space-y-3">
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <p className="text-sm text-slate-600 dark:text-zinc-400">{ta('loadingOptions')}</p>
            </div>
          ) : options.length === 0 ? (
            <p className="text-sm text-slate-500 dark:text-zinc-400 py-8 text-center">{ta('noOptions')}</p>
          ) : (
