...
import { apiBillingFailureJson } from '@/lib/api/api-error-response';
...
export async function POST(request: Request) {
  try {
    const secretKey = process.env.STRIPE_SECRET_KEY;
    if (!secretKey) {
      return apiBillingFailureJson(request, 'stripeNotConfigured', 500);
    }

    const body = await request.json().catch(() => ({}));
    const parsed = syncProfileBodySchema.safeParse(body);
    if (!parsed.success) {
      return apiBillingFailureJson(request, 'syncProfileSessionIdRequired', 400);
    }
    ...
    return apiBillingFailureJson(request, 'syncProfileFailed', 500);
  }
}
