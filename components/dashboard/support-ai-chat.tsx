...
function AgentThinking() {
  const steps = [
    { icon: '🔍', label: 'Analyse du compte…' },
    { icon: '⚡', label: 'Vérification en cours…' },
    { icon: '✍️', label: 'Rédaction de la réponse…' },
  ];
      ...
      <div className="max-w-[82%] sm:max-w-[72%]">
        ...
        {msg.created_at && (
          <p className="text-[10px] text-slate-400 dark:text-zinc-500 mt-1 pl-1">
            Agent REPUTEXA · {formatTime(msg.created_at)}
          </p>
        )}
      </div>
    </div>
...
export function SupportAiChat() {
  ...
  const refreshLists = useCallback(async () => {
    ...
    if (!openRes.ok) throw new Error(openJ.error ?? 'Erreur');
    if (!archRes.ok) throw new Error(archJ.error ?? 'Erreur');
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-slate-500 dark:text-zinc-400">Chargement de l'agent…</p>
        </div>
      </div>
    );
  }
        ...
        <div className="px-4 py-4 border-b border-slate-100 dark:border-zinc-800/70">
          <div className="flex items-center gap-2 mb-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 dark:text-zinc-500">REPUTEXA Support</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-4 scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-zinc-700">
          ...
          <div>
            ...
            <ul className="space-y-1">
              ...
              {ticketsOpen.map((tk) => {
                const fallback = t('ticketTitle', { date: formatShort(tk.updated_at) });
                        ...
                        {isRenaming ? (
                          ...
                          <>
                            ...
                            <p className={`text-[10px] mt-0.5 ${isActive ? 'text-indigo-200' : 'text-slate-400 dark:text-zinc-500'}`}>
                              Modifié {formatShort(tk.updated_at)}
                            </p>
                          </>
                        )}
                      </div>
                  ...
                  <li key={tk.id}>
                    ...
                    >
                      <p className="truncate">
                        {tk.title ?? t('ticketTitle', { date: formatShort(tk.updated_at) })}
                      </p>
                    </button>
                  </li>
                ))}
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 dark:text-zinc-500 mt-1">
                L'agent analyse votre compte dès le premier message.
              </p>
            </div>
            <button
              type="button"
            ...
            >
              {loadingChat ? (
                ...
                <div className="flex flex-col items-center justify-center h-full gap-4 py-12 text-center">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-slate-700 dark:text-zinc-200">
                      Bonjour, je suis votre Agent Expert REPUTEXA.
                    </p>
                ...
                messages.map((m, i) => <MessageBubble key={m.id ?? `${m.sender}-${i}`} msg={m} />)
              )}
