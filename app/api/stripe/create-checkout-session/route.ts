...
export async function POST(request: Request) {
  try {
    ...
    const successUrl = `${baseUrl}/api/stripe/checkout-success?session_id={CHECKOUT_SESSION_ID}&locale=${appLocale}&plan=${subscription.metadata?.planSlug ?? 'pulse'}&status=establishment_added&flow=add-establishment`;
