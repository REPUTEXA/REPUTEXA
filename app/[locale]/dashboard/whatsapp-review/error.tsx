...
export default function WhatsappReviewDashboardError({
  error,
  reset,
}: {
  ...
  const t = useTranslations('Dashboard.whatsappReviewError');
  useEffect(() => {
    console.error('[dashboard/whatsapp-review/error]', error);
  }, [error]);

  return (
    <div className="flex flex-col items-center justify-center min-h-[320px] gap-6 px-4 py-10 text-center max-w-lg mx-auto">
      <div className="w-14 h-14 rounded-2xl bg-red-50 dark:bg-red-950/30 flex items-center justify-center">
        <AlertTriangle className="w-7 h-7 text-red-500" />
      </div>
      <div>
        <p className="font-semibold text-slate-800 dark:text-zinc-100 text-base">{t('title')}</p>
