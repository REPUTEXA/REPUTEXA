...
function buildMasterPrompt(params: GenerateReviewParams): string {
  ...
  const nom = params.establishmentName.trim() || 'notre établissement';
  ...
  const negativeBlock = hasContact
    ? `
━━━ AVIS NÉGATIF (note ≤ 3/5) — BLOC CONTACT OBLIGATOIRE ━━━
...
Laisse une ligne vide avant la signature.
Signature : "La direction - ${nom}"
...
export async function generateText(params: {
  ...
  const { systemPrompt, userContent, temperature = 0.7, maxTokens = 1024 } = params;
  const anthropic = getAnthropic();
  const openai = getOpenAI();
