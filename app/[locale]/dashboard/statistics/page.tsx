...
export default async function StatisticsPage({ params }: Props) {
  ...
  if (supabaseUser) {
    const { data: profile } = await supabase
      .from('profiles')
      .select('subscription_plan, selected_plan, timezone')
    ...
    let reviewsQuery = supabase
      .from('reviews')
      .select('id, reviewer_name, rating, comment, source, response_text, ai_response, scheduled_at, created_at')
    ...
    reviews = (supabaseReviews ?? []).map((r) => {
      ...
      return {
        id: String(r.id),
        reviewerName: String(r.reviewer_name ?? t('anonymousReviewer')),
        rating: safeRating,
        comment: String(r.comment ?? ''),
        source: String(r.source ?? 'Unknown'),
        responseText: r.response_text ?? null,
        aiResponse: (r as { ai_response?: string | null }).ai_response ?? null,
        scheduledAt: (r as { scheduled_at?: string | null }).scheduled_at ?? null,
        createdAt: created,
      };
    });
  } else {
    ...
    reviews = prismaReviews.map((r) => {
      ...
      return {
        id: String(r.id),
        reviewerName: String(r.establishmentName ?? t('anonymousReviewer')),
        rating: safeRating,
  ...
  return (
    <div className="px-4 sm:px-6 md:px-8 py-6 sm:py-8 space-y-6 max-w-[1600px] mx-auto relative z-10">
      ...
      <WeeklyInsightSection planSlug={planSlug} locale={locale} />
