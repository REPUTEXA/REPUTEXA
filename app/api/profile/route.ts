...
/** Colonne `profiles.locale` (contrainte fr | en | es | de | it). */
function mapSiteLocaleToProfilesLocaleColumn(code: string): string {
  const c = code.toLowerCase();
  if (c === 'en-gb') return 'en';
  if (c === 'fr' || c === 'en' || c === 'es' || c === 'de' || c === 'it') return c;
  return 'fr';
}
...
export async function GET(request: Request) {
  ...
  // Plan actuel : même normalisation que le layout (subscription_plan puis repli selected_plan si sync en retard).
  const subscriptionPlanSlug = toPlanSlug(profile?.subscription_plan ?? null, profile?.selected_plan ?? null);
  ...
  return NextResponse.json({
    ...
    bananoPinConfigured: Boolean(profile?.banano_pin_hash),
    country: typeof profile?.country === 'string' ? profile.country : '',
    reputexaPlatformReviewSubmittedAt: profile?.reputexa_platform_review_submitted_at ?? null,
    reputexaDashboardEngineBadge: true,
    billingCurrency:
      typeof profile?.billing_currency === 'string' && profile.billing_currency.trim()
        ? profile.billing_currency.trim().toLowerCase()
        : null,
  });
}
...
export async function PATCH(request: Request) {
  ...
  if (typeof bodyLanguage === 'string' && VALID_LOCALES.includes(bodyLanguage)) {
    ...
    updates.preferred_language = bodyLanguage;
    updates.locale = mapSiteLocaleToProfilesLocaleColumn(bodyLanguage);
  }
  ...
  if (typeof reputexaDashboardEngineBadge === 'boolean') {
    updates.reputexa_dashboard_engine_badge = reputexaDashboardEngineBadge;
  }
