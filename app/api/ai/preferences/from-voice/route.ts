...
export async function POST(req: Request) {
  try {
    if (!process.env.OPENAI_API_KEY) {
      return apiJsonError(req, 'errors.openaiNotConfigured', 500);
    }
    ...
    if (!transcript || typeof transcript !== 'string' || !transcript.trim()) {
      return apiJsonError(req, 'errors.transcriptRequired', 400);
    }
    ...
    return apiMerchantAiJsonError(req, 'voicePreferencesUpdateFailed', 500);
  }
}
