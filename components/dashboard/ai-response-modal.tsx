...
export function AIResponseModal({ reviewId, reviewText, onClose, onResponded }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-0 md:p-4 bg-black/50 dark:bg-black/70 backdrop-blur-sm" onClick={onClose}>
      <div
        className="w-full max-w-lg max-h-[90vh] md:max-h-[85vh] overflow-y-auto rounded-t-2xl md:rounded-2xl bg-white dark:bg-slate-900 dark:backdrop-blur-md shadow-xl dark:shadow-[4px_6px_0_rgba(0,0,0,0.5)] border border-transparent dark:border-slate-800 dark:border-white/[0.07] animate-bottom-sheet-in pb-[env(safe-area-inset-bottom)]"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-800 dark:border-white/[0.07]">
          ...
          <button
            type="button"
            onClick={onClose}
            className="flex items-center justify-center min-w-[44px] min-h-[44px] p-2 rounded-xl text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 active:scale-[0.98] transition-transform"
            aria-label="Fermer"
          >
            <X className="h-5 w-5" />
          </button>
        ...
        <div className="p-4 space-y-3">
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <p className="text-sm text-slate-600 dark:text-slate-400">Génération des options...</p>
            </div>
          ) : options.length === 0 ? (
            <p className="text-sm text-slate-500 dark:text-slate-400 py-8 text-center">Aucune option disponible.</p>
          ) : (
              ...
              {options.map((opt, i) => {
                ...
                return (
                  <button
                    ...
                    disabled={!!sending}
                    className="w-full text-left p-4 rounded-2xl border border-slate-200 dark:border-slate-800 dark:border-white/[0.07] hover:border-sky-300 dark:hover:border-indigo-500/50 hover:bg-sky-50/50 dark:hover:bg-slate-800/50 active:scale-[0.99] transition-all disabled:opacity-70 min-h-[44px]"
                  >
