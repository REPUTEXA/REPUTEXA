-- Après 088 : le mode trial mettait subscription_status = 'trialing' à l'inscription,
-- ce qui faisait que verify-signup-otp renvoyait redirectTo dashboard sans stripeUrl ni étape CGU.
-- Règle : rester en pending jusqu'à checkout.session.completed (comme 041).
...
BEGIN
  ...
  IF mode_val = 'trial' THEN
    selected := 'zenith';
    plan := 'zenith';
    -- sub_status reste pending ; trial_ends_at défini par le webhook Stripe après checkout
  END IF;
