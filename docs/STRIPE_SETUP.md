...
| `STRIPE_PRICE_ID_VISION` | Stripe → Products → prix mensuel Vision (montants catalogue : `targets/settings.json` → `billing.plans.vision`) | `price_...` |
| `STRIPE_PRICE_ID_PULSE` | Stripe → Products → prix mensuel Pulse (idem) | `price_...` |
| `STRIPE_PRICE_ID_ZENITH` | Stripe → Products → prix mensuel Zenith (idem, ex. 179 EUR) | `price_...` |
...
**Important :** Crée 6 prix dans Stripe avec *Graduated Tiers* par devise : Vision, Pulse, Zenith × Mensuel, Annuel. Paliers : 1er 0%, 2e -20%, 3e -30%, 4e -40%, 5e+ -50%. L’annuel applique -20%. La quantité (nombre d’établissements) est gérée dans la subscription principale via `adjustable_quantity`.

**Devises catalogue :** EUR (défaut Europe), USD (`/en`), GBP (`/en-gb`), JPY (`/ja`), CNY (`/zh`), CHF (sélecteur), **CAD** (locales `fr-ca` / `en-ca` ou cookie `user-currency=cad`), **AUD** (locale `en-au` ou `aud`). Montants affichés : `targets/settings.json` → `billing.plans.*.prices`. Variables `.env` : préfixe `STRIPE_PRICE_ID_*` puis `_USD`, `_GBP`, `_JPY`, `_CNY`, `_CHF`, `_CAD`, `_AUD` (et `_ANNUAL_*` pour l’annuel). Sans ID pour une devise, repli automatique sur les prix EUR (log serveur).
