...
import { apiBillingJsonErrorDefaultLocale } from '@/lib/api/api-error-response';
...
import { billingCurrencyFromSubscription } from '@/lib/stripe-subscription';
...
import { safeIngestStripeBillingSignal } from '@/lib/omni-synapse';
...
export async function POST(request: Request) {
  ...
  if (!webhookSecret || !secretKey) {
    return apiBillingJsonErrorDefaultLocale('webhookSecretNotConfigured', 500);
  }
  ...
  if (!signature) {
    ...
    return apiBillingJsonErrorDefaultLocale('webhookNoSignature', 400);
  }
  ...
  try {
    ...
    return apiBillingJsonErrorDefaultLocale('webhookInvalidSignature', 400);
  }
  ...
  try {
          ...
          if (profiles?.length) {
            ...
            await admin
              ...
              .update({
                ...
                billing_currency: billingCurrencyFromSubscription(subscription),
              })
              .eq('id', profiles[0].id);
    ...
    if (event.type === 'customer.subscription.deleted') {
      ...
      if (admin) {
        const { data: profiles } = await admin
          .from('profiles')
          .select('id, email, language, establishment_name, full_name')
        ...
        if (profiles?.length) {
          ...
          await admin
            ...
            .eq('id', row.id);
          if (row.email?.trim()) {
            const { sendChurnSurveyEmail } = await import('@/lib/emails/churn-survey-email');
            void sendChurnSurveyEmail({
              to: row.email.trim(),
              locale: row.language,
    ...
    if (event.type === 'customer.subscription.updated') {
      ...
      if (justBecameActive) {
        ...
        if ((selectedPlan === 'vision' || selectedPlan === 'pulse') && currentPlan !== selectedPlan) {
          ...
          const currency = billingCurrencyFromSubscription(subscription);
          const targetPriceId = getStripePriceId(
            selectedPlan as PlanSlug,
            interval === 'year',
            currency
          );
    ...
    if (event.type === 'invoice.paid') {
      ...
      if (admin) {
        ...
        if (userId) {
          safeIngestStripeBillingSignal(admin, {
            userId,
    ...
    console.error('[stripe/webhook] Handler error detail:', msg);
    return apiBillingJsonErrorDefaultLocale('webhookHandlerFailed', 500);
  }
}
