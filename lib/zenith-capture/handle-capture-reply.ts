...
export async function handleCaptureReply(
  ...
  if (session.state === 'opt_in_sent') {
    ...
    if (isOptIn(trimmed)) {
      ...
      const { data: profile } = await supabase
        .from('profiles')
        .select('establishment_name, establishment_type, full_name')
      ...
      const invite = {
        ...
        establishmentType: profile?.establishment_type ?? null,
        firstName: null,
        lastPurchase: null,
      };
      const generated = await generatePostConsentFeedbackMessage(invite);
      const followUp = generated ?? staticPostConsentFallback(invite);
      await sendWhatsAppMessage(input.fromPhone, followUp);
      return true;
    }
    await sendWhatsAppMessage(
      input.fromPhone,
  ...
  if (session.state === 'collecting') {
    ...
    await sendWhatsAppMessage(
      input.fromPhone,
      `Voici une proposition courte qui reprend vos mots — uniquement si ça vous tente :\n\n« ${draft} »\n\nÇa vous va ? Répondez *Modifier* pour ajuster, ou *Publier* pour aller sur Google quand vous êtes prêt.`
    );
    return true;
  }
  ...
  if (session.state === 'proposition_sent') {
    ...
    if (isPublishRequest(trimmed)) {
      ...
      await sendWhatsAppMessage(
        input.fromPhone,
        `Parfait ! Le texte à publier est dans le message précédent (la proposition entre guillemets) : appui long pour le copier.`
      );
      await sendWhatsAppMessage(input.fromPhone, reviewUrl);
      await sendWhatsAppMessage(
        input.fromPhone,
        `Sur la page qui s'ouvre : collez, choisissez votre note, publiez. Merci beaucoup ! 🙏`
      );
      await sendWhatsAppMessage(
        input.fromPhone,
